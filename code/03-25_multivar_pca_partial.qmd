...
```{r r2}
#now do correlation with fiber strength
weather %>%
  ...
  mutate(r2 = map_dbl(data,
                  ~lm(strength_gtex ~ value,
                      data = .x) %>%
                    glance(.) %>% #model fit metrics
                    pull(r.squared) #get r squared
                  )) %>%
  arrange(desc(r2))
...
- All PCs are **orthogonal** (i.e., independent of each other).  Great for tackling multicollinearity
...
mod_pca %>%
  ...
  geom_hline(yintercept = 70) # 4 PCAs to get 70% explain
...
# Summary  
summary(lm_pca)
# only PC2 and PC3 are statistically significant
```
...
# Checking VIF
vif(lm_pca)
#no multicollinearity bc PCA takes it out. NICE benefit
```
...
ggplot(weather_postpca,
       aes(x = PC1, y = strength_gtex)) + 
         geom_point() +
         geom_smooth(method = "lm")
#did not show correlation bc PCA does not see strength_gtex, now that we can see it though there is no correlation.
```
